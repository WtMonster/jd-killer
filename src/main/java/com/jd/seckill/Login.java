...
public class Login {
    ...
    public static void login() throws IOException, URISyntaxException, InterruptedException {
        ...
        HttpUrlConnectionUtil.getQCode(Start.headers, "https://qr.m.jd.com/show?appid=133&size=147&t=" + now);
        ...
        Start.headers.put("Cookie", cookies);
        while (true) {
            ...
            String qrCode = HttpUrlConnectionUtil.get(Start.headers, checkUrl);
	...
	String qrCodeTicketValidation = HttpUrlConnectionUtil.get(Start.headers, "https://passport.jd.com/uc/qrCodeTicketValidation?t=" + ticket);
        stringListMap = Start.manager.get(url, requestHeaders);
